...
def load_beir_corpus(dataset_name, source='ir_datasets'):
    texts = []
    docs_ids = []
    ...
    if source == 'ir_datasets':
        ...
        # print("ir_datasets.registry._registered")
        # print(ir_datasets.registry._registered)
        ...
        for doc in tqdm(
            dataset.docs_iter(), total=dataset.docs_count(), desc="Loading beir documents from ir-datasets"
        ):
            texts.append(
                ftfy.fix_text(
...
def load_beir_queries(dataset_name, source='ir_datasets'):
    queries = {}
